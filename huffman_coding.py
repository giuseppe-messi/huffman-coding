...
nodes with lower frequencies take precedence in the tree's hierarchy, allowing for 
efficient encoding and decoding of the input text.


Following the construction of the frequency hashtable and the binary heap, the
build_huffman_tree method proceeds to build the Huffman tree. It does this by repeatedly 
selecting the two nodes with the lowest frequencies from the binary heap, merging them into 
a new node with their combined frequency, and inserting this new node back into the heap. 
This process continues until only one node remains in the heap, which represents the root 
of the completeHuffman tree. This tree is fundamental to the subsequent encoding and decoding 
of the input text.

The program's encode(tree, text) function is responsible for encoding the input text using
the Huffman tree generated by build_huffman_tree. It traverses the tree to find the binary
code for each character in the text, creating an encoded version of the input text that is 
more efficient for transmission and storage.
...
encoding. It then encodes and decodes the text, displaying both the encoded and decoded
versions. The user can decide to encode another text or exit the program. This loop provides 
hands-on experience with Huffman coding, demonstrating the core concepts of tree construction, 
encoding, and decoding. Overall, the program offers an intuitive and user-friendly interface 
for experimenting with Huffman coding.

In scenarios where the resulting Huffman tree consists of a single node, signifying that all 
characters in the encoded text are identical, the decode function offers an efficient handling 
mechanism. Instead of traversing the tree, it swiftly returns the repeated character, followed 
by the frequency of occurrence. This optimized approach ensures that even in situations of 
uniform characters, the program performs with exceptional efficiency.
